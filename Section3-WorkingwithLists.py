...
#could also use an iterator and the range() function
number_list = []
...
#could just turn the output of range() into a list directly
...
#make a new comprehension that builds a list of only the 
#odd numbers between 1 and 5 
a_list = [number for number in range(1,6) if number % 2 == 1]
a_list

#more compact compared with its traditional counterpart
a_list = []
for number in range(1,6):
    if number % 2 == 1:
...
#can be more than one set of for clauses in comprehension.

#using double for looping
rows = range(1,4)
cols = range(1,3)
for row in rows:
    for col in cols:
...
#you can also use tuple unpacking the row and col values
